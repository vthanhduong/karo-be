...
class Client:
    ...
    state: str = "awaiting_name"  # awaiting_name | lobby | game
...
class GameState:
    ...
    turn: str = "x"
    turn_deadline: datetime = field(default_factory=lambda: utcnow() + timedelta(seconds=MOVE_TIMEOUT_SECONDS))
...
class ConnectionManager:
    ...
    async def _cleanup_invites_for_client(self, client_id: str) -> None:
        async with self.lock:
            to_remove = [iid for iid, inv in self.invites.items() if inv.inviter_id == client_id or inv.target_id == client_id]
    ...
    try:
        async for text_message in websocket.iter_text():
            ...
            if msg_type == "invite_sent":
                ...
                # optional server filter; respond with filtered list
